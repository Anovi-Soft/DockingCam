...
namespace DockingCamera
{
    class PartCamera:BaseKspCamera
    {
        ...
        {
            return true;
            //var ray = new Ray(part.gameObject.transform.position, TargetHelper.Target.GetTransform().position);
            //RaycastHit hit;
            //if(Physics.Raycast(ray, out hit, AllowedDistance))
            //{
            //    return hit.collider.name.Equals(TargetHelper.Target.GetName());
            //}
            //return false;
            RaycastHit hit;
            if (Physics.Linecast(part.gameObject.transform.position, TargetHelper.Target.GetTransform().position, out hit))
            {
                List<string> names = new List<string>();
                Vessel vessel = null;
                if (TargetHelper.Target is Vessel)
                    vessel = TargetHelper.Target as Vessel;
                if (TargetHelper.Target is Part)
                    vessel = (TargetHelper.Target as Part).vessel;
                if (vessel != null)
                {
                    names = vessel.parts.Select(x => x.gameObject.name).ToList();
                    names.Add(vessel.gameObject.name);
                }
                else
                    names = new List<string> { TargetHelper.Target.GetName() };

                return names.Contains(hit.collider.gameObject.name);
            }
            else
                return false;
        }
